...
resource "aws_nat_gateway" "nat_gateway_1" {
  ...
  subnet_id     = aws_subnet.public_subnets["pub_subnet_1a"].id  # place the NAT gateway in the first public subnet
...
resource "aws_nat_gateway" "nat_gateway_2" {
  ...
  subnet_id     = aws_subnet.public_subnets["pub_subnet_1b"].id  # place the NAT gateway in the second public subnet
